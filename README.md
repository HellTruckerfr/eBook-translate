...
Outil de traduction automatique d'EPUBs anglais vers le français, conçu pour les romans de fantasy/science-fiction (initialement *My Vampire System*).
...
- **Export** — EPUB par arc, TXT, JSON (compatible MVS Audiobook)
...
2. `pyinstaller --onefile` — compile le backend en `mvs-backend.exe`
...
| `output_dir` | `Documents/MVS-Traduction` | Dossier de sortie des projets |

La configuration est stockée dans `%APPDATA%\MVS-Traduction\config.json`.
...
```
C:\MVS Traduction\mvs-backend.log
```

Fallback sur `%APPDATA%\MVS-Traduction\mvs-backend.log` si le dossier n'est pas accessible en écriture.

---

## Licence

Usage personnel. Projet non affilié à *My Vampire System* ou à ses auteurs.
